...
function calc_main(db)
    ...
    path_output_solver_ex = "./output/solver_ex/instance_work.txt"
    ...
    for item in list_instances 
        ...
        solver_ex(path_instance, path_output_solver_ex)
        write_solution_on_database(db, id, "exaustive", path_output_solver_ex)

       # solver_machine_rs(instance_name)
        solver_machine_rs(instance_name)
        write_solution_on_database(db, id, "3sat_rs", path_output_solver_rs)

        check_solution(db, id, path_output_solver_ex,path_output_solver_rs)
    end
...
function check_solution(db, id, path_output_solver_ex,path_output_solver_rs)
    ...
    is_valid = solver_ex_file_content == solver_machine_rs_file_content 
    is_sat = solver_ex_file_content != Set(["UNSAT"])
...
function solver_ex(path_instance, path_output_solver_ex)
    ...
    solver = test_massive_3sat.ExhaustiveSolver.new(path_instance)
    execution_time = @elapsed test_massive_3sat.ExhaustiveSolver.run!(solver)
